        ...
        if ($table == 'tx_cal_attendee') {
            ...
            if ($incomingFieldArray['fe_group_id'] > 0) {
                ...
                while ($row2 = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result2)) {
                    ...
                    if (false === $result) {
                        throw new \RuntimeException(
                            'Could not write attendee record to database: ' . $GLOBALS['TYPO3_DB']->sql_error(),
                            1431458136
                        );
                    }
                ...
                foreach ($tce->datamap['tx_cal_event'] as $eventUid => $eventArray) {
                    $eventArray['attendee'] = array_unique(array_merge(GeneralUtility::trimExplode(
                        ',',
                        $eventArray['attendee'],
                        1
                    ), $attendeeUids));

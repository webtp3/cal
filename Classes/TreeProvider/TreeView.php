    ...
    public function displayCalendar($PA, &$fobj)
    {
        ...
        if (!$disabled) {
            ...
            if (\TYPO3\CMS\Core\Utility\VersionNumberUtility::convertVersionNumberToInteger(TYPO3_version) >= 4006000) {
                ...
                $size = $config['autoSizeMax'] ? GeneralUtility::forceIntegerInRange(
                    count($itemArray) + 1,
                    GeneralUtility::forceIntegerInRange($size, 1),
                    $config['autoSizeMax']
                ) : $size;
            }
            ...
            if ($row['sys_language_uid'] && $row['l18n_parent']) { // the current record is a translation of another record
                ...
                if ($config['treeView'] and $config['foreign_table']) {
                    ...
                    if (\TYPO3\CMS\Core\Utility\VersionNumberUtility::convertVersionNumberToInteger(TYPO3_version) >= 4006000) {
                        ...
                        $height = $config['autoSizeMax'] ? GeneralUtility::forceIntegerInRange(
                            $treeItemC + 2,
                            GeneralUtility::forceIntegerInRange($size, 1),
                            $config['autoSizeMax']
                        ) : $size;
